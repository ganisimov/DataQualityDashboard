...
#' @param outputFile                (OPTIONAL) File see results of previous execution if resume is set
...
.runCheck <- function(checkDescription, 
  ...
  if (nrow(checks) > 0) {
    ...
    if (resume & nchar(outputFile) > 0 & file.exists(file.path(outputFolder, outputFile))) {
      dqdResults <- jsonlite::read_json(path = file.path(outputFolder, outputFile))
      checkResultsSaved <- lapply(dqdResults$CheckResults, function(cr) {
